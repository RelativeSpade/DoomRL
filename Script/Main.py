...
def get_latest_model(checkpoint_dir):
    # Regex to match the model filenames and extract the iteration number
    ...
    if not checkpoint_files:
        return None
    # Get the filename with the highest iteration number
    latest_checkpoint = max(checkpoint_files, key=lambda x: x[1])[0]
    print('Latest checkpoint:', latest_checkpoint)
    return os.path.join(checkpoint_dir, latest_checkpoint)


latest_model_path = get_latest_model(CHECKPOINT_DIR)
